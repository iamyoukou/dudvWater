...
//
//   glUniformMatrix4fv(uniform_model_skybox, 1, GL_FALSE,
//   value_ptr(model_main)); glUniformMatrix4fv(uniform_view_skybox, 1,
//   GL_FALSE, value_ptr(view_main));
...
void initPool() {
  ...
  //
  // shaderPool = glCreateProgram();
  // glAttachShader(shaderPool, vs);
  // glAttachShader(shaderPool, fs);
  //
  // glLinkProgram(shaderPool);
  // glGetProgramiv(shaderPool, GL_LINK_STATUS, &link_ok);
  //
  // if (link_ok == GL_FALSE) {
  //   std::cout << "Link failed: "
  //             << "model shaders" << '\n';
  ...
  uniform_tex = myGetUniformLocation(shaderPool, "tex");
  glUniform1i(uniform_tex, 1); // the 2nd parameter must be same as GL_TEXTUREi

  uniform_model_model = myGetUniformLocation(shaderPool, "model_model");
  uniform_view_model = myGetUniformLocation(shaderPool, "view_model");
  uniform_projection_model =
      myGetUniformLocation(shaderPool, "projection_model");
...
//   value_ptr(model_main)); glUniformMatrix4fv(uniform_view_water, 1, GL_FALSE,
//   value_ptr(view_main)); glUniformMatrix4fv(uniform_projection_water, 1,
//   GL_FALSE,
//                      value_ptr(projection_main));
//
//   // dudv texture
